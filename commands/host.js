...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const Message = await Channel.send({ content: `<@&${MentionRoleId}>`, embeds: [Embed], components: [Buttons] });
    ...
    Collector.on('collect', async btnInteraction => {
      ...
      if (btnInteraction.customId === 'cancel') {
        ...
        return btnInteraction.reply({ content: 'Training has been cancelled.', ephemeral: true });
      }
      ...
      if (btnInteraction.customId === 'end_training') {
        ...
        await Message.delete().catch(() => {});
        return btnInteraction.reply({ content: 'Training has ended and logged successfully.', ephemeral: true });
      }
    });
    ...
    ModalCollector.on('collect', async modalInteraction => {
      ...
      await modalInteraction.reply({ content: 'Training updated successfully.', ephemeral: true });
    });
